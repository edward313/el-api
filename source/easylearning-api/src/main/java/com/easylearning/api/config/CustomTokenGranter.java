    ...
    protected OAuth2AccessToken getAccessToken(ClientDetails client, TokenRequest tokenRequest) {
        ...
        try {
            if(SecurityConstant.GRANT_TYPE_STUDENT.equalsIgnoreCase(tokenRequest.getGrantType())
                    || SecurityConstant.GRANT_TYPE_EXPERT.equalsIgnoreCase(tokenRequest.getGrantType())) {
                String phoneOrEmail = tokenRequest.getRequestParameters().get("phone");
                return userService.getAccessTokenForCustom(client, tokenRequest, phoneOrEmail, password, tokenRequest.getGrantType(), this.getTokenServices());
            } else{
